...
- Bloom、難易度與誘答選項用來檢查品質，不應為了平均而扭曲題型。
...
```markdown
| 題號 | 題型 | 教材重點 | 教學目標/課綱對應 | 難度 | Bloom | 考風依據 | 備註 |
|---|---|---|---|---|---|---|---|
| 1 | 字音 | 本課生字 | 目標 1 | 容易 | 記憶 | 考古題常見字音題 |  |
```
...
- 難易度預設每 10 題為容易 2、中等 5、困難 3；30 題為容易 6、中等 15、困難 9。
- Bloom 預設使用記憶、理解、應用、分析、評鑑；創造不列入 Wayground 單選預設。
- 難易度與 Bloom 可以依科目考風微調，但必須在藍圖或審題紀錄說明原因。
...
誘答檢查併入本地一審，重點看困難題與高風險中等題：

- 每個錯誤選項對應哪一種學生可能犯的錯
- 多數中等或困難題至少有一到兩個錯誤選項會讓部分理解的學生猶豫
...
- 若是單一文法、單一公式或單一概念的 10 題專練，藍圖可簡化，但仍需列題號、考點、難度與答案檢查。
